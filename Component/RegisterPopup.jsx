...
//   // ✅ BACKEND API INTEGRATION (CORRECTED)
//   const handleSubmit = async (e) => {
//     e.preventDefault();
...
//       console.log("Saved user from backend:", savedUser);
...
const RegisterPopup = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // mark registered in localStorage so navbar can react
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center 
      ...
      <div className="relative bg-white/10 backdrop-blur-xl border border-white/20 
        ...
        <button
          onClick={onClose}
          className="absolute top-3 right-4 text-3xl font-bold text-white hover:text-pink-400 transition"
        >
          ×
        </button>
        ...
        <form className="text-white" onSubmit={handleSubmit}>
          ...
          <div className="flex gap-2 mt-1">
            <select
              value={countryCode}
              onChange={(e) => setCountryCode(e.target.value)}
              className="p-3 rounded-xl bg-white/10 border border-cyan-400/40 outline-none neon-input"
            >
              <option className="bg-amber-950" value="+91">+91</option>
              <option className="bg-amber-950" value="+1">+1</option>
              <option className="bg-amber-950" value="+44">+44</option>
